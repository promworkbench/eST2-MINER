...
//			fitness = MyPlaceStatus.UNKNOWN; //for debugging
		ArrayList<ESTPlace> newChildren = addValidChildrenToQueue(lastP, fitness);
		ESTPlace nextP = null;
...
//			System.out.println("Next Place: "+nextP.toTransitions(transitions));		
	...
	private ArrayList<ESTPlace> addValidChildrenToQueue(ESTPlace place, ESTPlaceStatus fitness) {
...
//			System.out.println("Depth-limit reached.");			
		...
		//add valid children based on fitness (first outgoing, then ingoing)
			...
			if((fitness != ESTPlaceStatus.MALFED && fitness != ESTPlaceStatus.OVERFED) || !hasSingleMaximalOutTransition(place)) { //asymetric pruning of overfed places
